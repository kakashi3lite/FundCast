...
from src.api.cache import get_cache, CacheKey
from src.api.database_optimization import get_optimized_db_session
from src.api.async_tasks import get_task_manager, TaskPriority
...
class TestCacheBenchmarks:
    ...
    async def test_cache_bulk_operations(self, benchmark_metrics):
        ...
        for size in sizes:
            ...
            print(f"\nSize {size}:")
            print(f"  Set: {set_stats['mean']:.4f}s ({size/set_stats['mean']:.0f} ops/sec)")
            print(f"  Get: {get_stats['mean']:.4f}s ({size/get_stats['mean']:.0f} ops/sec)")
    ...
    async def test_cache_memory_usage(self):
        ...
        for name, data in data_sizes:
            ...
            print(f"  L2 memory: {stats.get('l2_cache', {}).get('memory_usage', 0)} bytes")
...
class TestAPIBenchmarks:
    ...
    async def test_auth_endpoint_performance(self, benchmark_metrics):
        ...
        print(f"\nAuth Performance:")
        print(f"  Registration: {len(reg_tasks)/reg_time:.1f} req/sec (success: {reg_success:.2%})")
        print(f"  Login: {len(login_tasks)/login_time:.1f} req/sec (success: {login_success:.2%})")
