...
from test_integration import admin_user, test_db, test_user
...
from src.api.database import Company, Investment, Market, MarketPosition, Offering, User
...
def company_data_strategy(draw):
    ...
    return {
        ...
        "stage": draw(
            st.sampled_from(["Pre-seed", "Seed", "Series A", "Series B", "Later"])
        ),
    ...
    async def test_cache_consistency_properties(self, keys, values, ttl):
        ...
        for key, value in zip(keys, values):
            ...
            if isinstance(value, dict):
                assert (
                    cached_value == value
                    or json.loads(json.dumps(cached_value)) == value
                )
            else:
...
class TestAPIInvariantStateMachine(RuleBasedStateMachine):
    ...
    def create_company(self, user_id, company_data):
        ...
        response = self.client.post(
            "/api/v1/companies/", json=company_data, headers=headers
        )
        ...
        if response.status_code == 201:
            ...
            self.companies[company_id] = {**company_data, "owner_id": user_id}
    ...
    def create_market(self, market_data):
        ...
        response = self.client.post(
            "/api/v1/markets/", json=market_data, headers=headers
        )
