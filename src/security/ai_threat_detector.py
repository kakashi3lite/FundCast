...
class PromptInjectionDetector:
    ...
    async def _semantic_analysis(
        ...
        try:
            ...
            severity = (
                ThreatLevel.HIGH if similarity_score > 0.9 else ThreatLevel.MEDIUM
            )
...
class BehavioralAnomalyDetector:
    ...
    async def analyze_behavior(
        ...
        return {
            ...
            "threat_level": self._calculate_threat_level(
                anomaly_score, attack_indicators
            ),
        }
    ...
    async def _extract_behavioral_features(
        ...
        # Timing features
        features["avg_request_interval"] = self._calculate_avg_interval(
        ...
        features["vocabulary_diversity"] = self._calculate_vocabulary_diversity(
            session_data
        )
        features["sentiment_variance"] = self._calculate_sentiment_variance(
            session_data
        )

        # Automation indicators
        features["perfect_timing_ratio"] = self._calculate_perfect_timing_ratio(
            session_data
        )
        features["repetitive_pattern_score"] = self._calculate_repetitive_patterns(
            session_data
        )
    ...
    def _calculate_perfect_timing_ratio(self, session_data: Dict) -> float:
        ...
        consistent_intervals = sum(
            ...
            if interval > 0 and interval == round(interval, 1)
        )
    ...
    def _calculate_human_typo_score(self, session_data: Dict) -> float:
        ...
        for inp in inputs:
            ...
            # Common typos
            human_indicators += len(
                re.findall(r"teh|adn|hte|thier|recieve", text.lower())
            )
    ...
    async def _calculate_anomaly_score(
        self, features: Dict[str, float], baseline: Optional[Dict]
    ) -> float:
    ...
    def _calculate_threat_level(
        self, anomaly_score: float, attack_indicators: List[str]
    ) -> ThreatLevel:
...
class AIThreatDetector:
    ...
    async def analyze_request(self, request_data: Dict[str, Any]) -> ThreatAssessment:
        ...
        risk_score = self._calculate_risk_score(
            threat_level, overall_confidence, detected_patterns
        )
    ...
    async def _analyze_behavioral_patterns(
        self, user_id: Optional[str], session_data: Dict
    ) -> Dict[str, Any]:
    ...
    async def _analyze_content_anomalies(self, request_data: Dict) -> Dict[str, Any]:
        ...
        if any(header.lower().startswith("x-forwarded") for header in headers):
            proxy_count = sum(
                1 for header in headers if header.lower().startswith("x-forwarded")
            )
            if proxy_count > 5:
        ...
        return {
            "threat_detected": threat_detected,
            "threat_level": (
                ThreatLevel.MEDIUM if threat_detected else ThreatLevel.BENIGN
            ),
            "anomalies": anomalies,
    ...
    def _generate_cache_key(self, request_data: Dict) -> str:
        ...
        key_data = {
            ...
            "query_hash": hashlib.md5(
                str(request_data.get("query", "")).encode()
            ).hexdigest(),
    ...
    def _calculate_risk_score(
        self, threat_level: ThreatLevel, confidence: float, patterns: List[str]
    ) -> float:
    ...
    def _generate_mitigation_actions(
        self, threat_level: ThreatLevel, patterns: List[str]
    ) -> List[str]:
        ...
        if threat_level >= ThreatLevel.CRITICAL:
            ...
            actions.extend(
                ["log_security_event", "increase_monitoring", "apply_content_filter"]
            )
        elif threat_level >= ThreatLevel.LOW:
            actions.extend(["log_event", "passive_monitoring"])
