...
class SecurityFilter:
    ...
    def is_safe_file(self, file_path: Path) -> bool:
        ...
        if any(
            part.startswith(".") and part not in {".env.example", ".gitignore"}
    ...
    def sanitize_content(self, content: str) -> Tuple[str, bool]:
        ...
        for pattern in self.compiled_patterns:
            ...
            if matches:
                ...
                logger.warning(
                    "Sensitive content detected and redacted", pattern=pattern.pattern
                )
...
class SemanticSearch:
    ...
    def __init__(self, project_root: str = None):
        ...
        try:
            ...
            raise InferenceError(
                "Semantic search initialization failed", "sentence_transformers"
            )
    ...
    def _rate_limit_check(self, client_id: str) -> bool:
        ...
        self.rate_limiter[client_id] = [
            timestamp
            for timestamp in self.rate_limiter[client_id]
        ...
        for file_path in resolved_root.rglob("*"):
            try:
                ...
                if not str(file_path.resolve()).startswith(str(resolved_root)):
                    logger.warning(
                        "Path traversal attempt detected", file_path=str(file_path)
                    )
                    continue
    ...
    def _extract_file_content(self, file_path: Path) -> Optional[SearchResult]:
        ...
        try:
            ...
            if len(sanitized_content) > MAX_CONTEXT_LENGTH:
                sanitized_content = (
                    sanitized_content[:MAX_CONTEXT_LENGTH] + "... [TRUNCATED]"
                )
            ...
            logger.warning(
                "Failed to extract file content", file_path=str(file_path), error=str(e)
            )
            return None
    ...
    def _generate_embeddings(self, texts: List[str]) -> np.ndarray:
        ...
        try:
            ...
            truncated_texts = [
                text[:512] if len(text) > 512 else text for text in texts
            ]
            ...
            raise InferenceError(
                "Failed to generate embeddings", "sentence_transformers"
            )

    async def search_context(
        self, query: str, client_id: str = "default", max_results: int = 10
    ) -> List[SearchResult]:
        ...
        try:
            ...
            for file_result in self.file_cache.values():
                ...
                if cache_key not in self.embeddings_cache:
                    content_embedding = self._generate_embeddings(
                        [file_result.content]
                    )[0]
    ...
    async def get_file_context(self, file_path: str) -> Optional[SearchResult]:
        ...
        try:
            ...
            logger.warning(
                "File context retrieval failed", file_path=file_path, error=str(e)
            )
...
async def search_codebase_context(
    query: str, client_id: str = "default", max_results: int = 10
) -> List[SearchResult]:
