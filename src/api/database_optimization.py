...
from .cache import CacheKey, cache
...
class ConnectionPoolManager:
    ...
    def create_optimized_engine(
        self, database_url: str, pool_name: str = "default", read_only: bool = False
    ) -> AsyncEngine:
...
class OptimizedSession:
    ...
    async def execute_optimized(
        ...
        if (
            ...
            and self.query_cache.should_cache_query(query)
        ):
...
async def get_optimized_db_session(
    pool_name: str = "default", read_only: bool = False
) -> OptimizedSession:
    """Get optimized database session."""
    engine = pool_manager.get_engine(pool_name)

    async with AsyncSession(
        engine, expire_on_commit=False, autoflush=True, autocommit=False
    ) as session:
...
async def initialize_database_pools():
    ...
    pool_manager.create_optimized_engine(
        settings.DATABASE_URL, pool_name="default", read_only=False
    )

    # Read-only pool for analytics queries
    if hasattr(settings, "DATABASE_READ_URL"):
        pool_manager.create_optimized_engine(
            settings.DATABASE_READ_URL, pool_name="readonly", read_only=True
        )
