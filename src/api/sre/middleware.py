...
class SREMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            status_code = getattr(
                response, "status_code", 500 if error_occurred else 200
            )

            # Record metrics asynchronously
            asyncio.create_task(
                self._record_metrics(
                    request_context, duration_ms, status_code, error_occurred
                )
            )
    ...
    async def _record_metrics(
        ...
        try:
            ...
            await self._record_slo_measurements(
                request_context, duration_ms, is_success, status_code
            )
            ...
            if request.query_params.get("detailed") == "true":
                ...
                health_data["sre_metrics"] = {
                    ...
                    "system_metrics": {
                        ...
                        "memory_percent": dashboard_data["system_metrics"][
                            "memory_percent"
                        ],
                        "request_rate": dashboard_data["application_metrics"][
                            "request_rate"
                        ],
                        "error_rate": dashboard_data["application_metrics"][
                            "error_rate"
                        ],
                    },
                }
                ...
                if any(
                    slo["status"] == "critical"
                    for slo in dashboard_data["slo_status"].values()
                ):
