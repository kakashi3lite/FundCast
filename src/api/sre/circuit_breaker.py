...
class CircuitBreaker(Generic[T]):
    ...
    async def call(
        ...
        **kwargs,
        ...
        try:
            ...
            # Record success
            call_result = CallResult(success=True, duration=duration, result=result)
            ...
            call_result = CallResult(
                success=not is_failure, duration=duration, exception=e
            )
    ...
    def _can_attempt_call(self) -> bool:
        ...
        if self.state == CircuitState.CLOSED:
            ...
            if self.next_attempt_time and now >= self.next_attempt_time:
                ...
                self.success_count = 0
                logger.info(
                    "Circuit breaker transitioning to half-open", circuit=self.name
                )
                return True
            return False
    ...
    def _should_open_circuit(self) -> bool:
        ...
        if failure_rate >= (
            100
            - (self.config.failure_threshold / len(self.rolling_window.results)) * 100
        ):
            return True
...
async def circuit_breaker_health_check() -> Dict[str, Any]:
    ...
    for name, stat in stats.items():
        if stat.state == CircuitState.OPEN:
            ...
            issues.append(
                f"Circuit breaker '{name}' has high failure rate: {stat.failure_rate:.1f}%"
            )
