...
class TaskExecutor:
    ...
    async def retry_task(self, task: TaskDefinition, error: Exception) -> Optional[TaskDefinition]:
        ...
        if retry_policy.should_retry(current_retry, error):
            ...
            retry_task = TaskDefinition(
                ...
                metadata={**task.metadata, 'retry_count': current_retry + 1, 'original_task_id': task.task_id}
            )
    ...
    async def _worker_loop(self, worker_name: str):
        ...
        while self.is_running:
            try:
                ...
                logger.debug("Worker executing task", worker=worker_name, task_id=task.task_id)
                ...
                if result.status == TaskStatus.FAILED:
                    retry_task = await self.executor.retry_task(task, Exception(result.error))
                    if retry_task:
