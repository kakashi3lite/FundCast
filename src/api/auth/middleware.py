...
from .security import TokenBlacklist, verify_token
...
class InMemoryRateLimit:
    ...
    def is_allowed(self, key: str, limit: int, window: int) -> tuple[bool, int]:
        ...
        self.requests[key] = [
            timestamp for timestamp in self.requests[key] if now - timestamp < window
        ]
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def _memory_rate_limit(self, client_id: str) -> tuple[bool, int]:
        ...
        return in_memory_limiter.is_allowed(
            client_id, self.calls_per_minute, self.window_size
        )
