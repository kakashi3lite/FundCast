...
from .models import BillingCycle, SubscriptionStatus, SubscriptionTier, UserSubscription
...
class LemonSqueezyClient:
    ...
    async def create_checkout_url(
        ...
        if response.status_code != 201:
            raise HTTPException(
                status_code=500, detail=f"Failed to create checkout: {response.text}"
            )
        ...
        await self._track_checkout_created(
            user_id, tier_slug, billing_cycle, checkout_url
        )
    ...
    async def _handle_subscription_created(self, event_data: Dict, db: Session):
        ...
        tier = (
            ...
            .filter(SubscriptionTier.slug == tier_slug)
            .first()
        )
    ...
    async def _handle_subscription_updated(self, event_data: Dict, db: Session):
        ...
        subscription = (
            ...
            .filter(UserSubscription.external_subscription_id == external_id)
            .first()
        )
    ...
    async def _handle_payment_success(self, event_data: Dict, db: Session):
        ...
        subscription = (
            db.query(UserSubscription)
            .filter(UserSubscription.external_subscription_id == subscription_id)
            .first()
        )
    ...
    async def _track_checkout_created(
        self, user_id: str, tier_slug: str, billing_cycle: str, checkout_url: str
    ):
        """Track checkout creation for analytics"""
        # Implementation would integrate with analytics service
        print(f"Checkout created: {user_id} -> {tier_slug} {billing_cycle}")

    async def _track_subscription_created(
        self, user_id: str, tier_slug: str, billing_cycle: str
    ):
        ...
        for tier in sorted(tiers, key=lambda t: t.display_order):
            ...
            if tier.price_annual:
                annual_savings = SubscriptionPricing.calculate_savings(
                    tier.price_monthly, tier.price_annual
                )
            ...
            comparison.append(
                {
                    ...
                    "annual_price_display": (
                        f"${tier.annual_price_dollars:.0f}"
                        if tier.price_annual
                        else None
                    ),
