...
class User(Base):
    ...
    accredited_verified_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True)
    )
    accredited_expires_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True)
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "accredited_status IN ('unknown', 'verified', 'rejected', 'expired')"
        ),
...
class Offering(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "offering_type IN ('reg_cf', 'reg_a', 'rule_506b', 'rule_506c')"
        ),
        CheckConstraint(
            "status IN ('draft', 'pending_review', 'live', 'paused', 'completed', 'cancelled')"
        ),
...
class Investment(Base):
    ...
    offering_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("offerings.id"), nullable=False
    )
...
class MarketPosition(Base):
    ...
    avg_price: Mapped[int] = mapped_column(
